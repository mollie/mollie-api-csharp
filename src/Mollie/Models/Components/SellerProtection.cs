...
namespace Mollie.Models.Components
{
    ...
    public static class SellerProtectionExtension
    {
        public static string Value(this SellerProtection value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static SellerProtection ToEnum(this string value)
        {
            foreach(var field in typeof(SellerProtection).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is SellerProtection)
                    {
                        return (SellerProtection)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum SellerProtection");
        }
    }
