...
namespace Mollie.Models.Components
{
    ...
    public static class SalesInvoiceStatusExtension
    {
        public static string Value(this SalesInvoiceStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static SalesInvoiceStatus ToEnum(this string value)
        {
            foreach(var field in typeof(SalesInvoiceStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is SalesInvoiceStatus)
                    {
                        return (SalesInvoiceStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum SalesInvoiceStatus");
        }
    }
}