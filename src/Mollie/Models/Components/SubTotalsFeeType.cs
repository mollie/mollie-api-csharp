...
namespace Mollie.Models.Components
{
    ...
    public static class SubTotalsFeeTypeExtension
    {
        public static string Value(this SubTotalsFeeType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static SubTotalsFeeType ToEnum(this string value)
        {
            foreach(var field in typeof(SubTotalsFeeType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is SubTotalsFeeType)
                    {
                        return (SubTotalsFeeType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum SubTotalsFeeType");
        }
    }
