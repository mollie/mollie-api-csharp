...
namespace Mollie.Models.Requests
{
    ...
    public static class CreatePaymentCodeExtension
    {
        public static string Value(this CreatePaymentCode value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static CreatePaymentCode ToEnum(this string value)
        {
            foreach(var field in typeof(CreatePaymentCode).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is CreatePaymentCode)
                    {
                        return (CreatePaymentCode)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum CreatePaymentCode");
        }
    }
