...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum AvailableBalanceOpenSubtotalFeeType2
    {
    ...
    public static class AvailableBalanceOpenSubtotalFeeType2Extension
    {
        public static string Value(this AvailableBalanceOpenSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AvailableBalanceOpenSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(AvailableBalanceOpenSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AvailableBalanceOpenSubtotalFeeType2)
                    {
                        return (AvailableBalanceOpenSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AvailableBalanceOpenSubtotalFeeType2");
        }
    }
