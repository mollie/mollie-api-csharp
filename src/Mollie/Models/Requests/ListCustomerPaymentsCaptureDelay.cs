...
namespace Mollie.Models.Requests
{
    ...
    public static class ListCustomerPaymentsCaptureDelayExtension
    {
        public static string Value(this ListCustomerPaymentsCaptureDelay value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ListCustomerPaymentsCaptureDelay ToEnum(this string value)
        {
            foreach(var field in typeof(ListCustomerPaymentsCaptureDelay).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ListCustomerPaymentsCaptureDelay)
                    {
                        return (ListCustomerPaymentsCaptureDelay)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ListCustomerPaymentsCaptureDelay");
        }
    }
