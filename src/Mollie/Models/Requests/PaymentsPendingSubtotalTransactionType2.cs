...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum PaymentsPendingSubtotalTransactionType2
    {
    ...
    public static class PaymentsPendingSubtotalTransactionType2Extension
    {
        public static string Value(this PaymentsPendingSubtotalTransactionType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static PaymentsPendingSubtotalTransactionType2 ToEnum(this string value)
        {
            foreach(var field in typeof(PaymentsPendingSubtotalTransactionType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is PaymentsPendingSubtotalTransactionType2)
                    {
                        return (PaymentsPendingSubtotalTransactionType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum PaymentsPendingSubtotalTransactionType2");
        }
    }
