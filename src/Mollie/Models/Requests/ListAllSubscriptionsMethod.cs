...
namespace Mollie.Models.Requests
{
    ...
    {
        public static string Value(this ListAllSubscriptionsMethod value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ListAllSubscriptionsMethod ToEnum(this string value)
        {
            foreach(var field in typeof(ListAllSubscriptionsMethod).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ListAllSubscriptionsMethod)
                    {
                        return (ListAllSubscriptionsMethod)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ListAllSubscriptionsMethod");
        }
    }
