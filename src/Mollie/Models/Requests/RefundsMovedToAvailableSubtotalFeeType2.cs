...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum RefundsMovedToAvailableSubtotalFeeType2
    {
    ...
    public static class RefundsMovedToAvailableSubtotalFeeType2Extension
    {
        public static string Value(this RefundsMovedToAvailableSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static RefundsMovedToAvailableSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(RefundsMovedToAvailableSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is RefundsMovedToAvailableSubtotalFeeType2)
                    {
                        return (RefundsMovedToAvailableSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum RefundsMovedToAvailableSubtotalFeeType2");
        }
    }
