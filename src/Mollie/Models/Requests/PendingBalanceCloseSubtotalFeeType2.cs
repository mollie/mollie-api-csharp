...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum PendingBalanceCloseSubtotalFeeType2
    {
    ...
    public static class PendingBalanceCloseSubtotalFeeType2Extension
    {
        public static string Value(this PendingBalanceCloseSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static PendingBalanceCloseSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(PendingBalanceCloseSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is PendingBalanceCloseSubtotalFeeType2)
                    {
                        return (PendingBalanceCloseSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum PendingBalanceCloseSubtotalFeeType2");
        }
    }
