...
namespace Mollie.Models.Requests
{
    ...
    /// </remarks>
    /// </summary>
    public enum ListPaymentsCaptureDelay
    {
        [JsonProperty("... hours")]
        DotDotDotHours,
        [JsonProperty("... days")]
    ...
    public static class ListPaymentsCaptureDelayExtension
    {
        public static string Value(this ListPaymentsCaptureDelay value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ListPaymentsCaptureDelay ToEnum(this string value)
        {
            foreach(var field in typeof(ListPaymentsCaptureDelay).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ListPaymentsCaptureDelay)
                    {
                        return (ListPaymentsCaptureDelay)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ListPaymentsCaptureDelay");
        }
    }
