...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum TopupsMovedToAvailableSubtotalFeeType2
    {
    ...
    public static class TopupsMovedToAvailableSubtotalFeeType2Extension
    {
        public static string Value(this TopupsMovedToAvailableSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static TopupsMovedToAvailableSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(TopupsMovedToAvailableSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is TopupsMovedToAvailableSubtotalFeeType2)
                    {
                        return (TopupsMovedToAvailableSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum TopupsMovedToAvailableSubtotalFeeType2");
        }
    }
