...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum CorrectionsPendingSubtotalFeeType2
    {
    ...
    public static class CorrectionsPendingSubtotalFeeType2Extension
    {
        public static string Value(this CorrectionsPendingSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static CorrectionsPendingSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(CorrectionsPendingSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is CorrectionsPendingSubtotalFeeType2)
                    {
                        return (CorrectionsPendingSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum CorrectionsPendingSubtotalFeeType2");
        }
    }
