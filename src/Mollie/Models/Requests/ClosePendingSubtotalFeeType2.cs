...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum ClosePendingSubtotalFeeType2
    {
    ...
    public static class ClosePendingSubtotalFeeType2Extension
    {
        public static string Value(this ClosePendingSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ClosePendingSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(ClosePendingSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ClosePendingSubtotalFeeType2)
                    {
                        return (ClosePendingSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ClosePendingSubtotalFeeType2");
        }
    }
