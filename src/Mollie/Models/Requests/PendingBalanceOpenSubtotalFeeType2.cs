...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum PendingBalanceOpenSubtotalFeeType2
    {
    ...
    public static class PendingBalanceOpenSubtotalFeeType2Extension
    {
        public static string Value(this PendingBalanceOpenSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static PendingBalanceOpenSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(PendingBalanceOpenSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is PendingBalanceOpenSubtotalFeeType2)
                    {
                        return (PendingBalanceOpenSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum PendingBalanceOpenSubtotalFeeType2");
        }
    }
