...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum PaymentsMovedToAvailableSubtotalFeeType2
    {
    ...
    public static class PaymentsMovedToAvailableSubtotalFeeType2Extension
    {
        public static string Value(this PaymentsMovedToAvailableSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static PaymentsMovedToAvailableSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(PaymentsMovedToAvailableSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is PaymentsMovedToAvailableSubtotalFeeType2)
                    {
                        return (PaymentsMovedToAvailableSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum PaymentsMovedToAvailableSubtotalFeeType2");
        }
    }
