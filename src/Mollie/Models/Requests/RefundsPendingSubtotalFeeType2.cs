...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum RefundsPendingSubtotalFeeType2
    {
    ...
    public static class RefundsPendingSubtotalFeeType2Extension
    {
        public static string Value(this RefundsPendingSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static RefundsPendingSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(RefundsPendingSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is RefundsPendingSubtotalFeeType2)
                    {
                        return (RefundsPendingSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum RefundsPendingSubtotalFeeType2");
        }
    }
