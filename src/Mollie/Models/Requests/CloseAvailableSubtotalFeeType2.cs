...
namespace Mollie.Models.Requests
{
    ...
    /// </summary>
    public enum CloseAvailableSubtotalFeeType2
    {
    ...
    public static class CloseAvailableSubtotalFeeType2Extension
    {
        public static string Value(this CloseAvailableSubtotalFeeType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static CloseAvailableSubtotalFeeType2 ToEnum(this string value)
        {
            foreach(var field in typeof(CloseAvailableSubtotalFeeType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is CloseAvailableSubtotalFeeType2)
                    {
                        return (CloseAvailableSubtotalFeeType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum CloseAvailableSubtotalFeeType2");
        }
    }
